    ...
    while True:
        ...
        def splitCheck(player,deltaBank):
            ...
            if player.balance == -1:
                for i in range(len(players)):
                    if(done and players[i].bankID==player.bankID):
                        players.pop(i)
                        break
        ...
        for player in players:
            ...
            if player.calculate_hand_value() == 21 or player.surrender or len(player.hand)==5:
                ...
                if(player.balance==-1 and player.calculate_hand_value()==21):
                   splitCheck(player,player.bet*3/2)
                if(player.balance==-1 and len(player.hand)==5):
